...
class Discriminator(nn.Module):
    """
    Class for WGAN-GP discriminator, is a subclass of nn.Module.
    ...
    def __init__(self, channels_img, features_d):
        ...
        self.disc = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),  # no instancenorm in input layer
            self._block(features_d, features_d*2, 4, 2, 1),  # 16x16
            self._block(features_d*2, features_d*4, 4, 2, 1),  # 8x8
            self._block(features_d*4, features_d*8, 4, 2, 1),  # 4x4
            nn.Conv2d(features_d*8, 1, kernel_size=4, stride=2, padding=0),  # 1x1
        )  # no Sigmoid as WGAN disc output is unconstrained
    ...
    def _block(self, in_channels, out_channels, kernel_size, stride, padding):
        ...
        return nn.Sequential(
            ...
            nn.InstanceNorm2d(out_channels, affine=True),  # instancenorm replaces batchnorm for all layers
            nn.LeakyReLU(0.2),
        )
...
class Generator(nn.Module):
    """
    Class for WGAN-GP generator, is a subclass of nn.Module.
...
def test():
    ...
    assert gen(z).shape == (batch_size, in_channels, height, width)
