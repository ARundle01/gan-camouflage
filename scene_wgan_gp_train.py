...
from scene_utils import gradient_penalty
...
# Create dataloader
loader = DataLoader(forest, batch_size=BATCH_SIZE, shuffle=True, drop_last=True)
...
opt_gen = optim.Adam(gen.parameters(), lr=LEARN_RATE, betas=(0.0, 0.9))
opt_critic = optim.Adam(critic.parameters(), lr=LEARN_RATE, betas=(0.0, 0.9))
...
newfile = open(f"./wgan-gp/losses/num_critics {CRITIC_ITER}/epochs_{NUM_EPOCHS}.txt", "w")
newfile.close()
...
for epoch in range(NUM_EPOCHS):
    ...
    for batch_idx, (real, _) in enumerate(loader):
        ...
        for _ in range(CRITIC_ITER):
            ...
            # Calculate GP for discriminator
            gp = gradient_penalty(critic, real, fake, device=device)

            # Calculate combined disc loss, with GP
        ...
        if batch_idx % 100 == 0:
            ...
            with torch.no_grad():
                ...
                with open(f"./wgan-gp/losses/num_critics {CRITIC_ITER}/epochs_{NUM_EPOCHS}.txt", "a") as output:
                    output.write(f"{epoch+1}, {loss_critic:.4f}, {loss_gen:.4f}\n")

                # Save real and fake image grids
                torchvision.utils.save_image(real[:32], fp=f"./wgan-gp/real/num_critics {CRITIC_ITER}/epochs {NUM_EPOCHS}/wgan_gp_real_e{epoch+1}.png", normalize=True)
                torchvision.utils.save_image(fake[:32], fp=f"./wgan-gp/fake/num_critics {CRITIC_ITER}/epochs {NUM_EPOCHS}/wgan_gp_fake_e{epoch+1}.png", normalize=True)
